...
<form action="{{ route('admin.contact.all') }}" method="GET" class="admin-form">
        ...
        <div class="col-md-4 mr-5">
            <div class="form-group row">
                ...
                <select name="status" id="status" class="form-control col-md-8">     
                    ...
                    <option value="1" @if(request()->input('status') === '1') selected @endif>Repalyed</option>                            
                    <option value="0" @if(request()->input('status') === '0') selected @endif>Didn't repalyed</option>                            
                </select>
            </div>
        </div>
        <div class="col-md-4">
            <div class="form-group row">
                ...
                <select name="sort_by" id="sort_by" class="form-control col-md-8">
                    <option @if(request()->input('sort_by') === 'created_at' OR empty(request()->input('sort_by'))) selected @endif value="name">Created at</option>
                    <option @if(request()->input('sort_by') === 'name') selected @endif value="posts_count">Name</option>
                    <option @if(request()->input('sort_by') === 'status') selected @endif value="status">Status</option>   
                    <option @if(request()->input('sort_by') === 'title') selected @endif value="updated_at">Title</option>
                </select>
            </div>
        </div>
    </div>
