...
<nav aria-label="breadcrumb" class="my-3">
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <form action="{{route('admin.post.update', $post->slug)}}" method="post" class="admin-form" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <select name="status" class="form-control @error('status') is-invalid @enderror" id="status">
                                ...
                                <option value="0" @if(old('status', $post->status) === 0) selected @endif>Inactive</option>
                            </select>
                            @error('status')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <select name="Comment_able" class="form-control @error('Comment_able') is-invalid @enderror" id="Comment-able">
                                <option value="1" @if(old('comment-able', $post->comment_able) === 1) selected  @endif >Yes</option>
                                <option value="0" @if(old('comment-able', $post->comment_able) === 0) selected @endif >No</option>
                            </select>
                            @error('Comment_able')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                    ...
                    @if($post->post_cover != null)
                        <div class="row post-cover justify-content-center">
                            <div class="col-md-8">
                                ...
                                <a href="{{route('admin.post.cover', $post->slug)}}" class="btn btn-danger"><i class="fa fa-trash"></i> Delete</a>
                            </div>
                        </div>  
                    @endif
                    ...
                    <div class="row post-images justify-content-center">
                        @forelse ($post->media as $media)
                            <div class="col-md-4">
                                ...
                                <a href="{{route('admin.post.image', [$post->slug, $media->id])}}" class="btn btn-danger"><i class="fa fa-trash"></i> Delete</a>
                            </div>
                            @empty                                
                        @endforelse
                    </div>
