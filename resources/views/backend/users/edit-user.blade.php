                ...
                <div class="col-md-4">                    
                    @if ($user->user_image != null)
                        <div class="">
                            ...
                            <form action="{{route('admin.user.destroy.image', $user->username)}}" method="post" class="d-none" id="form-delete-user-image">
                                @csrf
                                @method('delete')
                            </form>                            
                        </div>
                    @else
                        <img src="{{asset('assets/default/user.jpg')}}" alt="{{$user->username}}">
                    @endif
                </div>
                <div class="col-md-8">
                    <h3 class="text-center mb-3">User information</h3>
                    <form action="{{route('admin.user.update', $user->username)}}" method="post" class="admin-form" enctype="multipart/form-data">
                        @csrf
                        @method('put')
                        <div class="row">
                            <div class="col-md-6">
                        ...
                        <div class="row">                            
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input id="mobile" type="number" class="form-control @error('mobile') is-invalid @enderror" name="mobile" value="{{ old('mobile', $user->mobile) }}" required  autofocus>
                                    @error('mobile')
                                        <span class="invalid-feedback" role="alert">
                                            <strong>{{ $message }}</strong>
                                        </span>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select name="receive_email" id="receive_email" class="form-control @error('receive_email') is-invalid @enderror">
                                        ...
                                        @else
                                            <option value="1" @if($user->receive_email == 1) selected @endif>Yes</option>                            
                                            <option value="0" @if($user->receive_email == 0) selected @endif>No</option> 
                                        @endif                            
                                    </select>
                                    @error('admin')
                                        <span class="invalid-feedback" role="alert">
                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="form-group">
                                    ...
                                    <textarea name="bio" id="bio" class="form-control @error('bio') is-invalid @enderror" rows="5">{{ old('bio', $user->bio) }}</textarea>
                                    @error('bio')
                                        <span class="invalid-feedback" role="alert">
                                            <strong>{{ $message }}</strong>
                                        </span>
                    ...
                    <form action="{{route('admin.user.update.password', $user->username)}}" method="post" class="admin-form-password">
                        @csrf
                        @method('put')
                        <div class="row">
                            <div class="col-md-6">
