...
<form action="{{ route('admin.category.all') }}" method="GET" class="admin-form">
        ...
        <div class="col-md-4">
            <div class="form-group row">
                ...
                <select name="sort_by" id="sort_by" class="form-control col-md-8">
                    <option @if(request()->input('sort_by') === 'name' OR empty(request()->input('sort_by'))) selected @endif value="name">Name</option>
                    <option @if(request()->input('sort_by') === 'posts_count') selected @endif value="posts_count">Posts count</option>
                    <option @if(request()->input('sort_by') === 'status') selected @endif value="status">Status</option>   
                    <option @if(request()->input('sort_by') === 'created_at') selected @endif value="updated_at">Created at</option>
                </select>
            </div>
        </div>
    </div>
