...
<nav aria-label="breadcrumb" class="my-3">
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <form action="{{route('admin.page.update', $page->slug)}}" method="post" class="admin-form" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                
                <div class="form-group">
                    <label for="summernote" class="form-label">Description</label>
                    <textarea name="description"  class="form-control @error('description') is-invalid @enderror" id="summernote" rows="3">{{old('description', $page->description)}}</textarea>
                    @error('description')
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <select name="status" class="form-control @error('status') is-invalid @enderror" id="status">
                                ...
                                <option value="0" @if(old('status', $page->status) === 0) selected @endif>Inactive</option>
                            </select>
                            @error('status')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                        ...
                        <div class="form-group">
                            ...
                            <select name="category" class="form-control @error('category') is-invalid @enderror" id="category">											
                                ...
                                @foreach($allCategories as $category)
                                    <option value="{{ $category->id }}" @if(old('category',$page->category_id) == $category->id) selected @endif> {{ $category->name }} </option>
                                @endforeach
                            </select>
                            @error('category')
                                <span class="invalid-feedback" role="alert">
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <select name="Comment_able" class="form-control @error('Comment_able') is-invalid @enderror" id="Comment-able">
                                <option value="1" @if(old('comment-able', $page->comment_able) === 1) selected  @endif >Yes</option>
                                <option value="0" @if(old('comment-able', $page->comment_able) === 0) selected @endif >No</option>
                            </select>
                            @error('Comment_able')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                ...
                <div class="form-group ">
                    <label for="post_cover" class="form-label">Page cover</label>
                    @if($page->post_cover != null)
                        <div class="row post-cover justify-content-center">
                            <div class="col-md-8">
                                <img src="{{asset('assets/pages/'. $page->post_cover)}}" alt="">
                                <a href="{{route('admin.page.cover', $page->slug)}}" class="btn btn-danger"><i class="fa fa-trash"></i> Delete</a>
                            </div>
                        </div>  
                    @endif
                ...
                <div class="form-group @error('images.*') error-image @enderror">
                    ...
                    <div class="row post-images justify-content-center">
                        @forelse ($page->media as $media)
                            <div class="col-md-4">
                                <img src="{{asset('assets/pages/'. $media->file_name)}}" alt="">
                                <a href="{{route('admin.page.image', [$page->slug, $media->id])}}" class="btn btn-danger"><i class="fa fa-trash"></i> Delete</a>
                            </div>
                            @empty                                
                        @endforelse
                    </div>
